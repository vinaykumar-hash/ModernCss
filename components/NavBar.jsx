...
const NavBar = () => {
  ...
  async function SearchItem(){
    ...
    DataCategory.forEach((el)=>{
      ...
      NewStyleName.classList.add("text-gray-800","font-semibold","text-xl","sm:px-6","px-2","tracking-wide","mt-4")
      ...
      data.forEach((innerel)=>{
        if(innerel[1] == el){
          ...
          NewSearchItem.classList.add("my-2" ,"rounded-lg" ,"py-2", "w-auto" , "sm:mx-4", "font-medium", "pl-4","hover:bg-gray-800","hover:text-navbarColor-400","text-gray-800","border","border-gray-800","border-dotted")
  ...
  return (
    ...
    <div className='text-center sticky top-0 h-auto bg-gray-800 z-50 shadow-md flex justify-center items-center px-2'>
        <input onChange={SearchItem} type="text" placeholder='Search any Componet' className='searchtext bg-gray-800 text-navbarColor-400 font-normal text-sm h-10 w-80 rounded-lg px-4 outline-none my-2 mr-2 tracking-wide'/>
        <button onClick={navigateToSearchRoute} className='searchbutton bg-gray-800 text-navbarColor-400 rounded-lg h-10 w-10 opacity-100 transition-all hover:opacity-100 fa fa-search'></button>
    </div>
    <div className='SearchResults cursor-default transition-all flex justify-center items-center flex-col  absolute top-0 opacity-0 z-40 w-screen sm:px-0 px-2 '>
      <div className='SearchResultTitle sm:w-2/5 w-full text-gray-800 text-xs bg-navbarColor-400 backdrop-blur-sm rounded-t-lg p-4 drop-shadow-md'>" Search Result " -</div>
      <div className='searchresultcontainer px-2 sm:w-2/5 w-full bg-navbarColor-400 rounded-b-lg text-sm backdrop-blur-sm transition-all max-h-60 overflow-scroll capitalize drop-shadow-2xl '>
      </div>
